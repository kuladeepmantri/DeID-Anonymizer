...
- Start: `make up` or `docker compose -f deploy/docker-compose.yml --project-directory . up -d --build`
- Stop: `make down` or `docker compose -f deploy/docker-compose.yml --project-directory . down -v`
- Logs: `make logs`
- Tests: `make test`
...
- MinIO bucket not found: the stack includes an init job (`createbuckets`) that creates the bucket; re-run `make up`
